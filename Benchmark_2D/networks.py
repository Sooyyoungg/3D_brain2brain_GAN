...
class Unet_Discriminator(nn.Module):
    ...
    def __init__(self, input_nc=1, ndf=64, kw=3, padw=1, n_layers=3, n_latent=2, output_nc=1, use_bias=True, embed=True, device='cuda:0'):
        ...
        self.head = nn.Sequential(*[Conv2dBlock(input_nc, ndf, stride=1, kernel_size=kw, norm='sn', activation='lrelu', padding=padw), nn.LeakyReLU(0.2, True)])
        nf_mult = 1
        nf_mult_prev = 1
        self.enc_layers = []
        ...
        for n in range(1, n_layers+1):  # gradually increase the number of filters
            ...
            self.enc_layers.append(nn.Sequential(*[
                Conv2dBlock(ndf * nf_mult_prev, ndf * nf_mult, stride=1, kernel_size=kw, norm='sn', activation='lrelu', pad_type='zero', padding=padw),
                nn.AvgPool2d(2),
